...
public class ValidationEditableComboBox extends Region {
	...
	public ValidationEditableComboBox(ValidationType type) {

		validationProperty = new ValidationProperty(type);
		initComponent();
	}

	public ValidationEditableComboBox(ValidationType type, String promptText) {
		...
		textBox.textProperty().bindBidirectional(
				validationProperty.get().nameProperty());
	}

	public ValidationEditableComboBox(ValidationType type, String promptText, ValidationProperty validation) {

		validationProperty = new ValidationProperty(type);
		initComponent();

		textBox.setPromptText(promptText);
		validationProperty.bindBidirectional(validation);
			...
			public void changed(ObservableValue<? extends String> observable,
				...
				if (!textBox.isDisabled() && textBox.isFocused() == true) {
					if (textBox.getText().length() == 0) {
						...
						List<ValidationMaster> resultList = new ValidationData()
								.getValidationList(textBox.getText().trim());
		...
		while (results.hasNext()) {
			...
			menuItem.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent actionEvent) {
					...
					validationProperty.set(result);
				}
			});
		}
	}

	public ValidationProperty validationProperty() {
